...
class WallStopTest(unittest.TestCase):
	def set_and_get(self,lf,ls,rs,rf):
		...
		with open("/dev/rtmotor_raw_l0","r") as lf,\
			...
			right=int(rf.readline().rtstrip())

		return left, right

	def test_io(self):
		left, right=self.set_set_and_get(400,100,100,0) #total:600
		self.assertTrue(left==0 and right==0,"can't stop")

		left, right=set_and_get(400,0,0,99) #total:499
		self.assertTrue(left!=0 and right!=0,"can't move again")

		left, right=set_and_get(150,0,200,150) #total:500
		self.assertTrue(left==0 and right==0,"can't stop")
